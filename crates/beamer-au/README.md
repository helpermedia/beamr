...
This crate provides native AUv3 support using `objc2` bindings to Apple's AudioToolbox framework. It bridges `beamer-core` abstractions to the Audio Unit plugin format:

- **Native AUv3**: Direct Objective-C integration via `objc2` (no wrapper or SDK dependencies)
- **AUAudioUnit subclass**: Full lifecycle management (allocate/deallocate render resources)
- **Parameter automation**: Complete `AUParameterTree` integration with KVO callbacks
- **MIDI support**: Legacy MIDI 1.0 and MIDI 2.0 UMP event processing
- **Real-time buffer management**: Zero-allocation render path with config-based allocation
...
- [Buffer Storage Optimization](https://github.com/helpermedia/beamer/blob/main/crates/beamer-au/BUFFER_STORAGE_OPTIMIZATION.md) - Memory optimization strategy using config-based allocation
