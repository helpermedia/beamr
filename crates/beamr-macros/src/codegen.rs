    ...
    let unit_consts: Vec<TokenStream> = ir
        ...
        .map(|nested| {
            ...
            quote! {
                ...
                pub const #const_name: ::beamr::core::params::UnitId = #unit_id;
            }
        })
        .collect();
...
fn generate_units_impl(ir: &ParamsIR) -> TokenStream {
    ...
    if !ir.has_nested() {
        ...
        return quote! {
            impl #impl_generics ::beamr::core::params::Units for #struct_name #ty_generics #where_clause {}
        };
    }
    ...
    quote! {
        impl #impl_generics ::beamr::core::params::Units for #struct_name #ty_generics #where_clause {
            fn unit_count(&self) -> usize {
                use ::beamr::core::param_types::Params;
            ...
            fn unit_info(&self, index: usize) -> Option<::beamr::core::params::UnitInfo> {
                use ::beamr::core::param_types::Params;
                if index == 0 {
                    return Some(::beamr::core::params::UnitInfo::root());
                }
...
fn generate_params_impl(ir: &ParamsIR) -> TokenStream {
    ...
    quote! {
        impl #impl_generics ::beamr::core::param_types::Params for #struct_name #ty_generics #where_clause {
            ...
            fn iter(&self) -> Box<dyn Iterator<Item = &dyn ::beamr::core::param_types::ParamRef> + '_> {
                #iter_impl
            }

            fn by_id(&self, id: ::beamr::core::types::ParamId) -> Option<&dyn ::beamr::core::param_types::ParamRef> {
                #by_id_impl
            }

            fn by_id_mut(&mut self, id: ::beamr::core::types::ParamId) -> Option<&dyn ::beamr::core::param_types::ParamRef> {
                self.by_id(id)
            }
...
fn generate_set_all_unit_ids(ir: &ParamsIR) -> TokenStream {
    ...
    quote! {
        fn set_all_unit_ids(&mut self, unit_id: ::beamr::core::params::UnitId) {
            #(#assignments)*
        }
    }
}
...
fn generate_nested_discovery(ir: &ParamsIR) -> TokenStream {
    ...
    let group_match_arms: Vec<TokenStream> = ir
        ...
        .map(|(idx, nested)| {
            ...
            quote! {
                #idx => Some((#name, &self.#field as &dyn ::beamr::core::param_types::Params)),
            }
        })
        .collect();
    ...
    let group_mut_match_arms: Vec<TokenStream> = ir
        ...
        .map(|(idx, nested)| {
            ...
            let name = &nested.group_name;
            quote! {
                #idx => Some((#name, &mut self.#field as &mut dyn ::beamr::core::param_types::Params)),
            }
        })
        .collect();
    ...
    quote! {
        ...
        fn nested_group(&self, index: usize) -> Option<(&'static str, &dyn ::beamr::core::param_types::Params)> {
            match index {
                #(#group_match_arms)*
                _ => None,
            }
        ...
        fn nested_group_mut(&mut self, index: usize) -> Option<(&'static str, &mut dyn ::beamr::core::param_types::Params)> {
            match index {
...
fn generate_iter(ir: &ParamsIR) -> TokenStream {
    let param_iters: Vec<TokenStream> = ir
        ...
        .map(|param| {
            let field = &param.field_name;
            quote! { &self.#field as &dyn ::beamr::core::param_types::ParamRef }
        })
        .collect();
...
fn generate_parameters_impl(ir: &ParamsIR) -> TokenStream {
    ...
    quote! {
        impl #impl_generics ::beamr::core::params::Parameters for #struct_name #ty_generics #where_clause {
            fn count(&self) -> usize {
                #count_impl
            }
            ...
            fn get_normalized(&self, id: ::beamr::core::types::ParamId) -> ::beamr::core::types::ParamValue {
                match id {
                    #(#get_match_arms)*
                    _ => {
            ...
            fn set_normalized(&self, id: ::beamr::core::types::ParamId, value: ::beamr::core::types::ParamValue) {
                match id {
                    #(#set_match_arms)*
                    _ => {
                        // Check nested
            ...
            fn normalized_to_string(&self, id: ::beamr::core::types::ParamId, normalized: ::beamr::core::types::ParamValue) -> String {
                use ::beamr::core::param_types::Params;
                self.by_id(id).map(|p| p.display_normalized(normalized)).unwrap_or_default()
            }

            fn string_to_normalized(&self, id: ::beamr::core::types::ParamId, string: &str) -> Option<::beamr::core::types::ParamValue> {
                use ::beamr::core::param_types::Params;
                self.by_id(id).and_then(|p| p.parse(string))
            }

            fn normalized_to_plain(&self, id: ::beamr::core::types::ParamId, normalized: ::beamr::core::types::ParamValue) -> ::beamr::core::types::ParamValue {
                use ::beamr::core::param_types::Params;
                self.by_id(id).map(|p| p.normalized_to_plain(normalized)).unwrap_or(0.0)
            }

            fn plain_to_normalized(&self, id: ::beamr::core::types::ParamId, plain: ::beamr::core::types::ParamValue) -> ::beamr::core::types::ParamValue {
                use ::beamr::core::param_types::Params;
...
fn generate_info(ir: &ParamsIR) -> TokenStream {
    ...
    if ir.has_nested() {
        ...
        quote! {
            fn info(&self, index: usize) -> Option<&::beamr::core::params::ParamInfo> {
